...
--local f, err = io.open(backlight_path .. "max_brightness")
--if not f then
	----TODO: naughty notify
--end
--local max_brightness = f:read("*a")
--f:close()
--local f, err = io.open(backlight_path .. "actual_brightness")
--local brightness = f:read("*a")
--f:close()
--local step = math.floor(tonumber(max_brightness) / 100)

--local function brightness(change)
	--local c = change * step
